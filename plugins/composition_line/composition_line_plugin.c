#include "composition_line_plugin.h"
#include <gtk/gtk.h>
#include <math.h>

static composition_line_data_t *data = NULL;
    ...
    if (data->st_obj) {
        ...
        if (p) {
            ...
            if (data->ed_obj != data->st_obj) {
                ...
                con = composition_line_create (data->st_port, data->ed_port);
...
#define SELECT_PLUGIN_RES_PATH "/tool_plugin/composition_line_plugin/"

static tool_plugin_t composition_line_plugin = {
    .resource_path = SELECT_PLUGIN_RES_PATH "composition_line_plugin.ui",
    .style_path = SELECT_PLUGIN_RES_PATH "composition_line_plugin.css",
    .button_name = "composition_line_btn",
    .menu_name = NULL,
...
tool_plugin_t *tool_plugin_init(void) {
    if (!data) {
        composition_line_plugin_register_resource();

        data = xcalloc (1, sizeof (composition_line_data_t));
        data->tmp = composition_line_create(NULL, NULL);
    }

    return &composition_line_plugin;
}
